...
PYTHONPATH=toolkit/src python -m unittest discover -s toolkit/tests -p 'test_*.py'
...
- The exact toolkit gitlink owns the source-preserving `addon-v1` contract,
  checker, migrator, and canonical templates. Run its `conventions migrate`
  command only on a clean child worktree, then use `conventions check`. A
  conventions rollout must not change Java sources, versions, tags, or sealed
  artifacts.
